...
pf <- brmsformula( weight ~ water * fertilizer + (1|plot:water:fertilizer))
get_prior(pf, data=pumpkins)
pp <- c(set_prior("normal(0, 10);", class="b"))

pfit <- brm(pf, data=pumpkins, family=gaussian(), prior=pp)
```
...
## brms with position-specific dispersion?

```{r new_brms, cache=TRUE}
baseball_bf2 <- brmsformula(Hits | trials(AtBats) ~ PriPos + weight + height + (1|PriPos/Player))
get_prior(baseball_bf2, data=batplus, family=binomial)
baseball_brms2 <- brm(baseball_bf2, data=batplus, family=binomial(), prior=baseball_bp)
```

----------------

```{r predict_brms2, echo=FALSE, fig.width=3*fig.dim, fig.height=2*fig.dim}
brms_pred2 <- inv_logit_scaled(fitted(baseball_brms2, scale='linear'))
plot_preds(brms_pred2, main="brms predictions, v2")
```
