...
Let's have a look:
```{r sim_mvn2, fig.width=1.5*fig.dim, fig.height=1.5*fig.dim}
nobs <- 100000
beta <- 0.7
sigma <- 1
xy <- data.frame(x = rnorm(nobs, mean=0, sd=1))
...
Use `mvtnorm::rmvnorm( )` to
simulate 10,000 random draws from
$$ (X_1, X_2, X_3) \sim \MVN\left(0, \Sigma\right) .$$
This will give you a $10^4 \times 3$ matrix.
First make histograms of each variable.
Then look at a `pairs( )` plot.
...
2. $$ \Sigma = \begin{bmatrix} 1 & 1 & 1 \\ 1 & 2 & 1 \\ 1 & 1 & 3 \end{bmatrix} . $$
