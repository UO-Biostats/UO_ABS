---
title: "Shrinkage: sparsifying priors"
author: "Peter Ralph"
...
c(train=sqrt(mean(resid(ols)^2)),
  test=ols_mse)
```
With an ordinary linear model, we got a root-mean-square-prediction-error of `r ols_mse` (on the *test* data),
...
1. Write a function `rexponormal(n, lambda)` that returns
...
2. Find, by trial-and-error, a value of $\lambda$
   so that this give you random numbers that are usually between $\pm 200$
   but are sometimes as big as $\pm 500$ or $\pm 1000$.
...
matplot(brms_xvals[,c(paste0("ols_", c("test", "train")), paste0("horseshoe_", c("test", "train")))],
        type='l', col=c(1, 1, 2, 2), lty=c(1, 2, 1, 2), ylab='root mean squared error', xlab='fold number')
...
```{r brmresultsB, echo=FALSE, fig.height=2*fig.dim, warning=FALSE, message=FALSE}
blm_samps <- fixef(brms_ols, summary=FALSE)
bayesplot::mcmc_intervals(blm_samps) + ggtitle("OLS") + xlim(-300, 600)
```
...
```{r brmresults2B, echo=FALSE, fig.height=2*fig.dim, warning=FALSE, message=FALSE}
bhs_samps <- fixef(brms_hs, summary=FALSE)
bayesplot::mcmc_intervals(bhs_samps) + ggtitle("horseshoe") + xlim(-300, 600)
```
...
Suppose we measure the expression of gene FLC
in 100 individuals with heights between 15cm and 100cm.
The mean expression level in plants of height $h$cm is $2.3 h$ RPKM,
with a standard deviation of 10 RPKM.
Simulate data, then make histograms of:
