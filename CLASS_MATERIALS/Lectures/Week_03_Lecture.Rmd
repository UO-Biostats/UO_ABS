...
*Challenge:* visualize *adult body size* by Family.
...
Try some different geoms,
with different aesthetics.

Use the [cheatsheet](https://rstudio.com/wp-content/uploads/2015/03/ggplot2-cheatsheet.pdf).


# Multivariate ANOVA


## The ANOVA model, again

The $k$th observation in the $j$th subset of the $i$th group is $X_{ijk}$,
$$\begin{equation}
...
- $\alpha_i$ is the mean deviation of group $i$ from $\mu$
- $\beta_{ij}$ is the mean deviation of the $j$th subset of group $i$ from $\mu + \alpha_i$, and
- $\epsilon_{ijk}$ is what's left over ("error", or "residual")

. . .

In words,
$$\begin{equation}
    \text{(price)} = \text{(group mean)} + \text{(subgroup mean)} + \text{(residual)}
\end{equation}$$
