...
airbnb <- read.csv("../Datasets/portland-airbnb-listings.csv")
airbnb$price <- as.numeric(gsub("$", "", airbnb$price, fixed=TRUE))
price <- airbnb$price[!is.na(airbnb$price)]
true_mean <- mean(price, na.rm=TRUE)
```

## 

```{r do_reps}
nvals <- c(10, 50, 100, 150, 200)
mean_error <- rep(NA, length(nvals))
for (k in seq_along(nvals)) {
    n <- nvals[k]
    reps <- replicate(1000, {
        x <- sample(price, size=n, replace=FALSE)
        abs(mean(x) - true_mean)
    })
    mean_error[k] <- mean(reps)
}
plot(nvals, mean_error, type='l')
```
