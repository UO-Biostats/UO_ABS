...
- Get familiar with course mechanics and resources.
- Overview of statistics and data science.
- Ask some questions of some data.
- Review introductory statistics concepts
- ... and practice being *active* users of these, with *simulation checks*.
...
figure with data -> model -> simulation
...
2. try some things from the website out (for instance: pulling down the course notes with git), and/or
3. move around.
...
So: in my sample of size $n=100$, the mean is $\bar x = 1.2$ and the SD is $s = 2.5$.
Is it reasonable the true mean is $\mu = 0$?

. . .

Well, the SE is $s / \sqrt{n} = 0.25$,
and the chance of getting a sample mean at least $1.2 / 0.25 = 4.8$ SEs above the true mean
is `pt(4.8, df=98, lower.tail=FALSE) = 2.845217e-06`.
So: no, it's not reasonable ($p = 2 \times 10^{-6}$, $t$-test with $df=98$).
...
text(-2.5, 0.03, expression(bar(x) == mu + t %*% frac(sigma, sqrt(n))), cex=3)
```
