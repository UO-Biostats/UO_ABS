...
![people using tools](images/using-tools.jpeg){width=80%}
...
See [the course website](https://petrelharp.github.io/UO_ABS).
...
layout(1:2)
hist(airbnb$price[!airbnb$instant_bookable], breaks=40, xlab='nightly price ($)', col=grey(.8), xlim=range(airbnb$price, finite=TRUE), main='instant bookable') 
hist(airbnb$price[airbnb$instant_bookable], breaks=40, xlab='nightly price ($)', col=grey(.8), main='not instant bookable')
```
...
then
$$\begin{equation}
    t(X - \mu) = \frac{\bar x - \mu}{\sigma} \approx \StudentsT(n-2) ,
\end{equation}$$
...
(mean(airbnb$price[fake_is_instant]) - 
       mean(airbnb$price[!fake_is_instant]))
```
...
- Harder to do.
- Confidence intervals needs another step.
...
```{r many conf_int, fig.height=2*fig.dim, echo=FALSE}
