...
```{r debug}
save(real_fit, file="temp.RData")
stan_trace(real_fit, pars=c("alpha", "beta", "sigma_xy", "sigma_eps", "x[1]", "y[1]", "x[2]", "y[2]"))
```

```{r osc_stan2, cache=TRUE}
osc_block2 <- "
data {
    int N;
    vector[N] X;
...
transformed parameters {
    ...
    y[1] = y1;
    for (k in 1:(N-1)) {
        x[k+1] = x[k] + alpha * y[k] + sigma_xy * dx[k];
        y[k+1] = y[k] - beta * x[k] + sigma_xy * dy[k];
    }
}
model {
    x1 ~ normal(0, 5);
...
"
osc_model2 <- stan_model(model_code=osc_block2)
```


```{r run_osc_for_reals, cache=TRUE, dependson="load_osc_data"}
real_fit2 <- sampling(osc_model2,
                    data=list(N=nrow(osc_data),
                              X=osc_data[,1],
                              Y=osc_data[,2]),
                    iter=1000, chains=3,
