...
```{r resids}
hist(
   resid(lm(Femur ~ SVL_cm, data=lizards)),
   breaks=20
)
hist(
   resid(lm(MeanFingerArea ~ SVL_cm, data=lizards)),
   breaks=20
)
```
...
```{r plitit2, fig.height=1.8*fig.dim, fig.width=2*fig.dim}
...
```{r pairs}
mcmc_pairs(lfit, regex_pars="Femur.*")
````
...
Remaining female lizards of average size on Pine Cay had an average finger pad area that was `r round(100 * mean(pp_ratio))`% larger (95% CI of `r `quantile(pp_ratio, probs = 0.025)-`r `quantile(pp_ratio, probs = 0.975)`%) than before the hurricane. This difference of `r mean(pp[,2] - pp[,1])` square centimeters represents roughly three-quarters of a standard deviation of finger pad area. Similarly, males of average length ...
