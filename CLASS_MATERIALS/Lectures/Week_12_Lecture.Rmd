...
for (j in 1:nrow(co2)) {
    ...
    # e.g., monthly_mean[7] will be the average difference in co2 concentration from the overall mean
    # across the seven Julys in this chunk of data from 7 years
    monthly_mean <- rep(NA, 12)
    for (k in 1:12) {
        monthly_mean[k] <- with(subset(co2, nearest_seven),
                                mean(average[month == k] - mean(average, na.rm=TRUE), na.rm=TRUE))
    }
    # or, 
    #   monthly_mean <- tapply(co2$average[nearest_seven], co2$month[nearest_seven], mean, na.rm=TRUE) - mean(co2$average, na.rm=TRUE)

    # note: we don't need all 12 months
    co2$our_trend[j] <- co2$average[j] - monthly_mean[co2$month[j]]
}
