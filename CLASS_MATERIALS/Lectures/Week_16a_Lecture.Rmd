...
$$F_{ik} = c_{1k}y_{i1} + c_{2k}y_{i1} + c_{3k}y_{i2} + c_{4k}y_{i3} + ... + c_{pk}y_{ip}$$
where

- `i` indexes the observation
- `k` indexes the new factor
...
## Multivariate distance and dissimilarity indices and metrics
...
- Difference is that it starts with an **n-by-n matrix** of object dissimilarities
...
- PCoA is a distance-based ordination method that can be performed via the capscale() function in VEGAN. 
- You can also use cmdscale() in the base installation, but you will need to produce a distance matrix from the original data. 
- PCoA is also called classical multidimensional scaling or metric multidimensional scaling. 
- The major benefit of PCoA is the ability to choose a different distance measure. When Euclidean distance is used, PCoA is the same as PCA.
...
```{r, eval=FALSE, echo=TRUE}
PCA.res<-prcomp(data.frame) 
```

- where data.frame is the name of the data frame variable
...
## R Interlude
- You should store the results as a variable, an object of class prcomp (dont worry about what class means) that contains specific items. 
- Most of the time you will want to plot the results manually using the values provided, to customize the sample colors and other parameters, but to get a quick plot you can use the biplot() command:
...
# Multidimensional Scaling (MDS and nMDS)
## Multidimensional Scaling (MDS and nMDS)
...
- VEGAN is a very useful package written for R
- Primarily designed for ecological data analysis
- We actually use it quite a bit for genomic data such as RNA-seq data.
- Analyzing the distribution and abundance of species in space and time is surprisingly similar to the distribution of gene transcripts in space and time.
- As a reference, find the full VEGAN manual in our course Repo 
- PCoA is a distance-based ordination method that can be performed via the `capscale()` function in VEGAN. 
- You can also use `cmdscale()` in the base installation, but you will need to produce a distance matrix from the original data. 

## R Interlude

- First off, make sure you installed the vegan package with all dependencies.
