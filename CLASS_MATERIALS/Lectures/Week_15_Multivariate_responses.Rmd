...
mcmc_intervals(ps,
       pars=c(
  paste0("cor_nest__",
     outer(c("mass6", "tars6", "mass14", "tars14"),
           c("mass6", "tars6", "mass14", "tars14"),
           paste, sep="_Intercept__"), "_Intercept")))
```
...
kresid1 <- lapply(lapply(bm1_kfold$fits[1:5], resid), function (x) x[,"Estimate",])
kresid2 <- lapply(lapply(bm2_kfold$fits[1:5], resid), function (x) x[,"Estimate",])
```

## Compute root-mean-squared-error

```{r get_rmse, echo=FALSE, fig.width=3*fig.dim, fig.height=1.5*fig.dim}
rmse1 <- as.data.frame.table(sapply(kresid1, function (x) sqrt(colMeans(x^2))))
names(rmse1) <- c("variable", "fold", "rmse")
rmse1$model <- 1
rmse2 <- as.data.frame.table(sapply(kresid2, function (x) sqrt(colMeans(x^2))))
names(rmse2) <- c("variable", "fold", "rmse")
rmse2$model <- 2
...
Strong support for nest effects:
(quantify??)
