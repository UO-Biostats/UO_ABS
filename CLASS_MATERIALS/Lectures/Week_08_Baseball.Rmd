...
empty <- data.frame(
            ...
            AtBats=10000)
(fm_post <- predict(fm, newdata=empty))
```
...
```{r plot_first_pred}
par(mar=c(5,7,1,1)+.1)
plot(fm_post[,"Estimate"] / empty$AtBats, 1:nrow(empty), xlim=c(0.1, 0.3), xlab='batting avg', ylab='', yaxt='n', type='n')
segments(x0=fm_post[,"Q2.5"]/empty$AtBats, x1=fm_post[,"Q97.5"]/empty$AtBats, y0=1:nrow(empty), col='red')
points(fm_post[,"Estimate"] / empty$AtBats, 1:nrow(empty), pch=20)
axis(2, at=1:nrow(empty), labels=empty$PriPos, las=2)
```
...
fm_samps <- posterior_predict(fm, newdata=empty)
right_fielders <- fm_samps[,match("Right Field", empty$PriPos)]
catchers <- fm_samps[,match("Catcher", empty$PriPos)]
...
ba_order <- rank((batting$Hits / batting$AtBats)[ut])
plot(ba_order, (batting$Hits / batting$AtBats)[ut], type='n', ylim=c(0.0, 0.4))
